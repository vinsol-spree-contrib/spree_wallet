Spree::Payment.class_eval do
  validates :amount, :numericality => { :less_than_or_equal_to => lambda { |payment| [payment.order_user_or_by_email.store_credits_total, payment.order_remaining_total].min }}, :if => lambda { |payment| payment.order_user_or_by_email && payment.wallet? && payment.amount_changed? }, :allow_blank => true
  validates :amount, :numericality => { :less_than_or_equal_to => lambda { |payment| payment.order_remaining_total }, :greater_than_or_equal_to => 0 }, :unless => :wallet?, :if => :amount_changed?, :allow_blank => true
